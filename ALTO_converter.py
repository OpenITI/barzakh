"""Convert ALTO XML transcriptions generated by the UMD pipeline
...
def get_all_region_types_in_document(root, nsmap):
    ...
    """
    all_regions = set()
    for region in root.xpath("//default:TextRegion|//default:TextBlock", namespaces=nsmap):
...
def get_region_type(region_element):
    ...
    if region_type:
        region_type = re.findall(r"type:(\w+)", region_type)[0]
    else: # ALTO
        region_type = region_element.get("ID")
...
##            print("Trying to get the bounding box from lines")
##            line_bboxes = []
##            line_xss = []
##            line_yss = []
##            for line in el.xpath("//default:TextLine", namespaces=nsmap):
##                line_bbox, line_xs, line_ys = get_bounding_box(line, nsmap, element_type="line")
##                line_bboxes.append(line_bbox)
##                line_xss += line_xs
##                line_yss += line_ys
##            
##            min_x = min([b[0] for b in line_bboxes])
##            max_x = max([b[1] for b in line_bboxes])
##            min_y = min([b[2] for b in line_bboxes])
##            max_y = max([b[3] for b in line_bboxes])
##            
##            return [min_x, max_x, min_y, max_y], line_xss, line_yss
##        else:
    ...
    if "," in points:     # in PAGE xlm: [x,y x,y x,y]
        xs = [int(coord.split(",")[0]) for coord in points.split(" ")]
    else:                 # in ALTO:     [x y x y x y]
        xs = [float(p) for p in points.split(" ")[::2]]
    min_x = min(xs)
    max_x = max(xs)
    if "," in points:
        ys = [int(coord.split(",")[1]) for coord in points.split(" ")]
    else:
        ys = [float(p) for p in points.split(" ")[1::2]]
    min_y = min(ys)
    max_y = max(ys)
...
def parse_lines(root, nsmap, include_regions, fp, exclude_regions=[],
    ...
    for text_line in root.xpath("//default:TextLine", namespaces=nsmap):
        ...
        if not line_text:  # ALTO has the line text in CONTENT 
            line_text = []
            for word in text_line.findall("default:String", namespaces=nsmap):
                line_text .append(word.get("CONTENT"))
            line_text = " ".join(line_text)
            line_d["text"] = line_text
        else:
...
def get_image_fn(root, nsmap):
    ...
    image_fn = ""
    try:
        image_fn = root.find(".//default:Page", nsmap).get("imageFilename")
    except Exception as e:
        print("Error extracting image filename", e)
    if not image_fn:
        try:
            el = root.find(".//default:fileName", nsmap)
            image_fn = etree.tostring(el, method="text", encoding='utf-8')
            image_fn = image_fn.decode('utf-8').split("/")[-1]
        except Exception as e:
            print("Error extracting image filename", e)
            return ""
    #print(image_fn)
...
def post_process(text, line_segment_separator):
    ...
    # remove line numbers:
    text = re.sub(line_segment_separator+r"\d+\n", "\n", text)
...
def download_transcriptions(escr, download_folder, output_type="pagexml", projects=None,  
    ...
    if projects:
        projects = [re.sub(r"\W", "-", p.lower()) for p in projects]
        ...
        if not document_names or doc.name in document_names:
            ...
            if not projects or project in projects:
                ...
                for t in transcriptions:
                    ...
                    normalized_transcription_layer = re.sub(r"""[/<>:"\\|?*]""", "_", t.name)
    ...
    with open(meta_fp, mode="r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            if row["which regions to include"]:
                include_regions = re.split(r"[,;] *", row["which regions to include"])
        ...
        for book_uri in multivol:
            ...
            for fp, vol in sorted(multivol[book_uri]):
                ...
                bodies.append(re.sub(r"PageV\d+", f"PageV{vol:02d}", body))
                os.remove(fp)
                i += 1
            with open(outfp, mode="w", encoding="utf-8") as file:
...
def add_alto_files(meta_fp, download_folder, dest_folder, coll_id="AOCP2",
                   include_regions=[], exclude_regions=["Footnotes",],
                   min_line_overlap=20, line_segment_separator="   ",
                   include_image_name=True, reorder_pages=False,
                   skip_orphan_lines=True, reconvert=False, 
                   main_text_region="Main", language_code="ara",
                   default_transcription_layer=None, common_meta={}):
    """Add files exported from Yufei's OCR pipeline to barzakh
    
    Args:
        meta_fp (str): path to the tsv file containing the metadata
            (headers: Hollis_search_terms, to_be_scanned, Comment1, Comment2, Comment3, Author,
                      Title1, Title2, title_transcr, author_transcr, Edition_info, Hollis_permalink, Local_storage, URI,
                      Split_or_not, Google_Drive_path, Comment4, Comment5, change_uri_to)
        download_folder (str): path to the folder to which the transcriptions were downloaded
        dest_folder (str): path to the output folder
        coll_id (str): the collection ID that will become the first part of the version ID
        include_regions (list): a list of regions from which text needs to be extracted
        exclude_regions (list): a list of regions from which text needs to be dropped
        ...
        main_text_region (str): the name of the region that contains the
            main text of the page; defaults to "Main"
    """
    # collect the metadata from the OCR spreadsheet:
    meta_d = dict()
    with open(meta_fp, mode="r", encoding="utf-8") as file:
        reader = csv.DictReader(file, delimiter="\t")
        for row in reader:
            ...
            meta_d[uri] = row
            if "_V" in uri:
                meta_d[re.sub(r"_V\d+", "", uri)] = row
    print(len(meta_d), "uris with metadata")

    i = 0
    multivol = dict()
    for uri in os.listdir(transcr_folder):
        print(uri)
        # only process directories that contain transcriptions:
        book_folder = os.path.join(transcr_folder, uri)
        if not os.path.isdir(book_folder):
            continue
        i += 1

        # skip files that are already in the corpus:
        if "CORPUS/BARZAKH" in row and row["CORPUS/BARZAKH"]:
            print("skipping", uri, ": already in", row["CORPUS/BARZAKH"])
            continue
        ...
        try:
            ...
            print("no metadata found for", uri)
            row = dict()
        if "change_uri_to" in row and row["change_uri_to"]:
            uri = row["change_uri_to"]
        elif "CHANGE URI TO" in row and row["CHANGE URI TO"]:
            uri = row["CHANGE URI TO"]
        fn = f"{uri}.{coll_id}0{timestamp}{i:03d}-{language_code}1"
        outfp = f"{dest_folder}/{fn}"

        # skip this folder if it was already converted
        # (unless you want to reconvert):
        if not reconvert and os.path.exists(outfp):
            continue

        # deal with multivolume texts later:
        if "_V" in uri:
            uri, vol = uri.split("_V")
            if not uri in multivol:
                multivol[uri] = []
            multivol[uri].append([outfp, int(vol)])
        elif "NUMBER OF VOLUME" in row and row["NUMBER OF VOLUME"]:
            vol = row["NUMBER OF VOLUME"]
            if not uri in multivol:
                multivol[uri] = []
            multivol[uri].append([outfp, int(vol)])
        ...
        first_page = 0
        if "PAGE NUMBER OF FIRST IMAGE" in row and row["PAGE NUMBER OF FIRST IMAGE"]:
            first_page = int(row["PAGE NUMBER OF FIRST IMAGE"])

        # convert the xml files into a single mARkdown file:
        convert_folder(book_folder, outfp,
                       include_regions=["main_text_block"],
                       exclude_regions=[],
                       page_offset=0, min_line_overlap=20,
                       extension="xml", line_segment_separator="   ",
                       include_image_name=True, skip_orphan_lines=True,
                       first_page=first_page, transcription_meta=dict(),
                       main_text_region="main_text_block"
                       )
        ...
        # create metadata files if there is metadata:
        if not row:
            continue

        # fill in relevant YML fields:
        if "Hollis_permalink" in row:
            try:
                based = re.findall(r'https?:"([^"]+)', row["Hollis_permalink"])[0]
            except:
                based = ""
        elif "BASED LINK (WORLDCAT)" in row:
            based = row["BASED LINK (WORLDCAT)"]
        else:
            based = ""
        link = ""
        if "DOWNLOAD LINK" in row:
            link = row["DOWNLOAD LINK"]
        issues = "UNCORRECTED_OCR"
        if "ISSUES" in row:
            issues = row["ISSUES"]
        notes = escript_message + "¶    This file was created on "
        #notes += datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
        ...
        # create a book YML file if the metadata contains book relations / title transcription:
        relations = ""
        if "BOOK RELATIONS" in row and row["BOOK RELATIONS"]:
            relations = row["BOOK RELATIONS"]
        if "title_transcr" in row:
            title_transcr = row["title_transcr"]
        elif "TITLE_TRANSCRIPTION" in row:
            title_transcr = row["TITLE_TRANSCRIPTION"]
        else:
            title_transcr = ""
        book_uri = ".".join(uri.split(".")[:2])
    ...
    for book_uri in multivol:
        ...
        for fp, vol in sorted(multivol[book_uri]):
            ...
            header, body = text.split("#META#Header#End#")
            if i == 0:
                outfp = re.sub(r"_V\d+", "", fp)
                outfp = outfp.split("-")[0] + "Vols-" + outfp.split("-")[1]
                os.replace(fp+".yml", outfp+".yml")
                with open(outfp+".yml", mode="r", encoding="utf-8") as file:
                    yml_s = file.read()
                yml_s = re.sub(os.path.split(fp)[-1], os.path.split(outfp)[-1], yml_s)
...
##    transcr_folder = "eScriptorium_alto"
##    common_meta={
...
##        if not os.path.isdir(book_folder):
##            continue
##        outfp = f"temp/{uri}.AOCP20250612{i:03d}-{language_code}1"
##        i += 1
##        print(outfp)
##        convert_folder(book_folder, outfp,
##                       include_regions=["main_text_block"],
...
##                       extension="xml", line_segment_separator="   ",
##                       include_image_name=True, skip_orphan_lines=True,
##                       first_page=0, transcription_meta=dict(),
##                       main_text_region="main_text_block"
##                       )
    ...
    add_alto_files(meta_fp, transcr_folder, dest_folder, coll_id=coll_id,
                   include_regions=["main_text_block"], 
                   min_line_overlap=20, line_segment_separator="   ",
                   include_image_name=True, reorder_pages=False,
                   skip_orphan_lines=True, reconvert=False, 
